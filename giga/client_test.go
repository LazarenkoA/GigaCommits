...
func Test_prompt(t *testing.T) {
	prompt := new(Client).prompt("ru")
	assert.Equal(t, "Ты программист. Проанализируй отправленый тебе результат команды \"git diff\" и сгенерируйте КРАТКОЕ описание ИЗ ОДНОГО ПРЕДЛОЖЕНИЯ для команды git commit. Необходимо учесть приведенные ниже спецификации:\nЯзык сообщения: ru\nИсключите все ненужно. Весь ваш ответ будет передан непосредственно в git commit.", prompt)
}
